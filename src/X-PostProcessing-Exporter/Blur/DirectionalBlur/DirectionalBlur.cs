...
namespace XPostProcessing_PI
{
    ...
    public class DirectionalBlur : MonoBehaviour
    {
        ...
        private static string _DirectionX = "_DirectionX";
        private static string _DirectionY = "_DirectionY";
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destiantion)
        {
            if (shadowMaterial)
            {
                var Radius = BlurRadius / source.width / Iteration;

                shadowMaterial.SetFloat(_BlurRadius, BlurRadius / source.width);
                shadowMaterial.SetInt(_Iteration, Iteration);

                var x = Mathf.Sin(Angle / 180.0f * Mathf.PI) * Radius;
                var y = Mathf.Cos(Angle / 180.0f * Mathf.PI) * Radius;
                shadowMaterial.SetFloat(_DirectionX, x);
                shadowMaterial.SetFloat(_DirectionY, y);
